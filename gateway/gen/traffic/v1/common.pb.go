...
// How a source delivers a capture — the one distinction the gateway must act on, and
// the only thing it can't work out for itself at OpenSession (both shapes have zero
// bytes stored at that point; afterwards a pcap's size tells them apart).
//
// Which *tool* produced a capture is not this enum's business: that is `source`, a free
// string, because a closed enum cannot name a third-party module (ADR-0010).
type SourceShape int32

const (
	SourceShape_SOURCE_SHAPE_UNSPECIFIED SourceShape = 0
	// Streams raw packets (+ a TLS key-log) for the gateway to decode: a browser, an
	// Android app capture, an imported pcap.
	SourceShape_SOURCE_SHAPE_PCAP SourceShape = 1
	// Pushes flows it has already decoded (PushFlows): mitmproxy, a third-party module.
	// Such a session is live from the moment it opens — there is no pcap to decode.
	SourceShape_SOURCE_SHAPE_FLOWS SourceShape = 2
)

// Enum value maps for SourceShape.
var (
	SourceShape_name = map[int32]string{
		0: "SOURCE_SHAPE_UNSPECIFIED",
		1: "SOURCE_SHAPE_PCAP",
		2: "SOURCE_SHAPE_FLOWS",
	}
	SourceShape_value = map[string]int32{
		"SOURCE_SHAPE_UNSPECIFIED": 0,
		"SOURCE_SHAPE_PCAP":        1,
		"SOURCE_SHAPE_FLOWS":       2,
	}
)

func (x SourceShape) Enum() *SourceShape {
	p := new(SourceShape)
	*p = x
	return p
}

func (x SourceShape) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SourceShape) Descriptor() protoreflect.EnumDescriptor {
	return file_traffic_v1_common_proto_enumTypes[0].Descriptor()
}

func (SourceShape) Type() protoreflect.EnumType {
	return &file_traffic_v1_common_proto_enumTypes[0]
}

func (x SourceShape) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SourceShape.Descriptor instead.
func (SourceShape) EnumDescriptor() ([]byte, []int) {
...
type Session struct {
	...
	// The tool that produced this capture: a built-in ("chrome", "firefox", "android",
	// "mitmproxy"), "import" for a pcap import, or a module's own name. Free text — see
	// SourceShape for why this is not an enum. "" for a session recorded before this
	// field existed and whose legacy source kind could not be mapped.
...
const file_traffic_v1_common_proto_rawDesc = "" +
	...
	"\vSourceShape\x12\x1c\n" +
	"\x18SOURCE_SHAPE_UNSPECIFIED\x10\x00\x12\x15\n" +
	"\x11SOURCE_SHAPE_PCAP\x10\x01\x12\x16\n" +
	"\x12SOURCE_SHAPE_FLOWS\x10\x02*O\n" +
	"\bFileKind\x12\x19\n" +
...
var file_traffic_v1_common_proto_goTypes = []any{
	(SourceShape)(0),       // 0: traffic.v1.SourceShape
...
var file_traffic_v1_common_proto_depIdxs = []int32{
	2,  // 0: traffic.v1.Session.status:type_name -> traffic.v1.SessionStatus
	14, // 1: traffic.v1.Session.metadata:type_name -> traffic.v1.Session.MetadataEntry
	...
	15, // 9: traffic.v1.Flow.metadata:type_name -> traffic.v1.Flow.MetadataEntry
	...
	16, // 14: traffic.v1.WsMessage.metadata:type_name -> traffic.v1.WsMessage.MetadataEntry
