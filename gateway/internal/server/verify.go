package server

import (
	"context"
	"fmt"
	"log"
	"sort"

	trafficv1 "gitlab.com/nklyshko/traffic-deck/gateway/gen/traffic/v1"
	"gitlab.com/nklyshko/traffic-deck/gateway/internal/decode"
)

// verifyRecordedLive tshark-decodes the session's pcap (without persisting the result)
// and compares the just-recorded live flows against it, logging any differences — the
// verification pass for record-live mode, where the authoritative tshark decode on close
// is otherwise skipped. Failures are logged rather than returned: the session is already
// persisted and closed, and a broken verification must not fail the close.
func (i *Ingest) verifyRecordedLive(ctx context.Context, sid string, ls *liveSession, keylogPath string) {
	pcapLocal, ok := i.obj.LocalPath(pcapKey(sid))
	if !ok {
		log.Printf("verify %s: pcap not available locally; skipping tshark verification", sid)
		return
	}
	ds, err := decode.Decode(ctx, i.tshark, pcapLocal, keylogPath)
	if err != nil {
		log.Printf("verify %s: tshark decode failed: %v", sid, err)
		return
	}
	batch := make([]*trafficv1.Flow, 0, len(ds.Flows))
	for _, f := range ds.Flows {
		batch = append(batch, flowToProto(f))
	}
	verifyLiveVsTshark(sid, ls.protoFlows(), batch)
}

// verifyLiveVsTshark compares the live-decoded flows against the authoritative tshark
// decode and logs any differences, so a live-decoder gap (a missed or extra flow) is
// surfaced rather than silent. Flows are matched by a natural key
// (protocol/method/authority/path) counted per side — live and tshark assign independent
// ids, so exact id matching isn't possible. Returns the human-readable difference lines
// (empty when they match).
func verifyLiveVsTshark(sessionID string, live, tshark []*trafficv1.Flow) []string {
	liveCounts := countByFlowKey(live)
	tsharkCounts := countByFlowKey(tshark)

	seen := map[string]bool{}
	var diffs []string
	for k, tc := range tsharkCounts {
		seen[k] = true
		if lc := liveCounts[k]; lc != tc {
			diffs = append(diffs, fmt.Sprintf("%s: live=%d tshark=%d", k, lc, tc))
		}
	}
	for k, lc := range liveCounts {
		if !seen[k] {
			diffs = append(diffs, fmt.Sprintf("%s: live=%d tshark=0 (live-only)", k, lc))
		}
	}
	sort.Strings(diffs)

	if len(diffs) == 0 {
		log.Printf("verify %s: live decode matches tshark (%d flows)", sessionID, len(tshark))
		return nil
	}
	log.Printf("verify %s: live decode differs from tshark (live=%d, tshark=%d flows); %d key(s) differ:",
		sessionID, len(live), len(tshark), len(diffs))
	for _, d := range diffs {
		log.Printf("verify %s:   %s", sessionID, d)
	}
	return diffs
}

// countByFlowKey tallies flows by a stable natural key. WebSocket/custom flows key on
// protocol+authority (they have no method/path), so a per-endpoint count comparison still
// catches missing connections.
func countByFlowKey(flows []*trafficv1.Flow) map[string]int {
	out := map[string]int{}
	for _, f := range flows {
		key := f.GetProtocol() + " " + f.GetMethod() + " " + f.GetAuthority() + f.GetPath()
		out[key]++
	}
	return out
}
